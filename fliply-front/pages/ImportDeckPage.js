...
export class ImportDeckPage {
    ...
    return `
      <div class="animate-fade">
        <div class="page-header">
          <h1>Importar Deck</h1>
          <p style="color: var(--text-secondary); margin-top: 4px;">Cole o código de um deck compartilhado</p>
        </div>
          ...
          <div style="margin-bottom: var(--space-lg);">
            ...
            <div style="display: flex; gap: var(--space-sm);">
              <input
                ...
                style="flex: 1; padding: 12px 16px; border: 2px solid var(--border-color); border-radius: 8px; font-size: 16px; font-family: monospace; background: var(--surface);"
                maxlength="50"
              />
  ...
  async _search() {
    ...
    try {
      ...
      this._showPreview()
      Toast.success('Deck encontrado!')
    } catch (error) {
      this._showError(error.message || 'Deck não encontrado')
      Toast.error('Erro ao buscar deck')
    } finally {
      this.loading = false
    }
  }
  ...
  async _import() {
    ...
    try {
      ...
      Toast.error(error.message || 'Erro ao importar deck')
      const importBtn = this.container.querySelector('#import-btn')
  ...
  _showPreview() {
    ...
    this.container.querySelector('#deck-icon').textContent = deck.deckIcon || '📚'
    ...
    cardsContainer.innerHTML = deck.cards.slice(0, 5).map((card, idx) => `
      <div style="padding: 12px; background: var(--surface); border-radius: 6px; border-left: 4px solid ${deck.deckColor || '#7C3AED'};">
